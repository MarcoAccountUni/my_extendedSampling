...
from custom_esm.layers.blocks import UnifiedTransformerBlock
...
class TransformerStack(nn.Module):
    ...
    def last_attention(
        ...
        Returns:
            attention: The last attention tensor of shape (batch_size, sequence_length, sequence_length).
        """
        *batch_dims, _ = x.shape
        if chain_id is None:
            chain_id = torch.ones(size=batch_dims, dtype=torch.int64, device=x.device)
        for block in self.blocks[:-1]:
            x = block(x, sequence_id, affine, affine_mask, chain_id)
        # On the last block, instead of performing the usual forward pass, we stop at the attention matrix (mean among heads)
        attention = self.blocks[-1].attention(x, sequence_id, affine, affine_mask, chain_id)
        return attention
    ...
    def layers_attention(
        ...
        Args:
            x (torch.Tensor): The input tensor of shape (batch_size, sequence_length, d_model).
            layers (list[int]): The layers from which attention must be explicitly computed.
            sequence_id (torch.Tensor): The sequence ID tensor of shape (batch_size, sequence_length).
            affine (Affine3D | None): The affine transformation tensor or None.
            affine_mask (torch.Tensor | None): The affine mask tensor or None.
            chain_id (torch.Tensor): The protein chain tensor of shape (batch_size, sequence_length).
        ...
        *batch_dims, _ = x.shape
        ...
        for layer, block in enumerate(self.blocks):
            if layer in layers:
                attention = block.attention(x, sequence_id, affine, affine_mask, chain_id)
