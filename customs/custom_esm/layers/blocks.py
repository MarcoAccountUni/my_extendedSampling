...
from custom_esm.layers.attention import (
    FlashMultiHeadAttention,
    MultiHeadAttention,
)
from custom_esm.layers.geom_attention import (
    GeometricReasoningOriginalImpl,
)
from custom_esm.utils.structure.affine3d import Affine3D
...
class UnifiedTransformerBlock(nn.Module):
    ...
    def attention(
        ...
        -------
        torch.Tensor[float]
            The attention tensor after applying the transformer block operations free of the last ffn.
        """
        # For the moment, the return of the attention matrix is possible only for:
        # self.use_plain_attn = True & use_flash_attn = False
        # self.use_geom_attn = False
        # TODO: add implementation for FlashMultiHeadAttention and GeometricReasoningOriginalImpl
        assert self.use_plain_attn, 'UnifiedTransformerBlock.attention() not implemented yet for geometric attention!'
        return self.attn.attention(x, sequence_id)
